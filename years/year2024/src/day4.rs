...
fn inline_left(input: &[u8], line: usize, pos: usize) -> u16 {
    match (
        input[pos + (line * SIZE) - 1],
        input[pos + (line * SIZE) - 2],
        input[pos + (line * SIZE) - 3],
    ) {
        (M, A, S) => 1,
        _ => 0,
    }
...
fn up(input: &[u8], line: usize, pos: usize) -> u16 {
    match (
        input[pos + ((line - 1) * SIZE)],
        input[pos + ((line - 2) * SIZE)],
        input[pos + ((line - 3) * SIZE)],
    ) {
        (M, A, S) => 1,
        _ => 0,
    }
...
fn up_right(input: &[u8], line: usize, pos: usize) -> u16 {
    match (
        input[(pos + 1) + ((line - 1) * SIZE)],
        input[(pos + 2) + ((line - 2) * SIZE)],
        input[(pos + 3) + ((line - 3) * SIZE)],
    ) {
        (M, A, S) => 1,
        _ => 0,
    }
